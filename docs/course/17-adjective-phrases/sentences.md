...
**Items 5 and 6 are daggered and they are the same gap.** `AdjP > Cl/complement`
is on [../difficulty.md](../difficulty.md)'s list assigned to lesson 34;
`AdjP > PP/complement` is not on any list and is equally absent. Both need a
model check before either can be written, and the check is one question.

# Lesson 33 — Coordination between clauses · sentences

Drafted 28 August 2026. See [README.md](README.md).

**This lesson is sound and the changes are small.** Seven _and_, three _but_, no
_or_. Three distinct asked-shapes.

**No sentence has a comma, and that is deliberate.** Lesson 39 is built almost
entirely on comma-plus-coordinator sentences of exactly this shape, so the two
lessons are a controlled pair. It is the only punctuation contrast in the course
and it is worth protecting.

## Sentences

| #   | Sentence                                             | The step                                        |
| --- | ---------------------------------------------------- | ----------------------------------------------- |
| 1   | The kettle boiled and the lights dimmed.             | two intransitive clauses, joined                |
| 2   | The tide turned but the ferry waited.                | _but_, which sets the halves against each other |
| 3   | We can wait or we can leave.                         | **_or_**, the third coordinator                 |
| 4   | The rain eased and the streets were passable.        | the second half is a linking frame              |
| 5   | The engine stalled and the driver called a mechanic. | one half intransitive, one transitive           |
| 6   | The clerk read the minute but the board proceeded.   | both halves transitive                          |
| 7   | The talks resumed and the strike ended.              | back to the plainest join                       |
| 8   | The snow melted and the river rose.                  | two irregular verbs, short halves               |
| 9   | That lock rusted and the gate was immovable.         | a determiner that is not an article             |
| 10  | The quartet rehearsed but the audience left.         | close on _but_                                  |

## Notes

Item 3 adds the missing coordinator. It also uses a modal, which lesson 24
taught, so the composition costs nothing.

**Both halves stand alone**, and that is what separates coordination from
subordination. _The kettle boiled._ and _The lights dimmed._ are each sentences;
lesson 29's _because the gate was locked_ is not. Running that test across
lessons 29 and 33 is the cleanest way to make the difference concrete.

Two further facts worth an author knowing and not worth a sentence each:
coordinated clauses cannot swap freely when the order carries meaning — _The pipe
froze and the ceiling cracked_ is not the reverse — and the coordinator cannot
move, unlike an adverbial clause's marker, which travels with its clause to the
front.

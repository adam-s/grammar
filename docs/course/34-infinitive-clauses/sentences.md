...
**Items 4, 5 and 8 are daggered for different reasons.** Item 8 is on
[../difficulty.md](../difficulty.md)'s list: `AdjP > Cl/complement`, assigned to
lesson 34 and built nowhere. Items 4 and 5 are a probe finding — zero infinitive
clauses with an overt subject in the course — and probably representable rather
than missing, since nothing forbids a `Cl` from holding a subject. The check is
one question and should be asked before the sentences are written.

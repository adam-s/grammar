...
Restored 24 September 2026: this was removed by accident in c514264, while
the code and the browser checks still cited it as the design. Undo has since
shipped; see [undo.md](undo.md).
...
Those belong to the dev-only replay route, and to nothing else. The one piece
of history a learner does get is undo, one step at a time: see
[undo.md](undo.md).
